...
COORDINATE_PARSING_SCHEMA = [
    ...
    "[Cc]oord\|[0-9]+\|[0-9]+\|[0-9]+\|[NS]\|[0-9]+\|[0-9]+\|[0-9]+\|[EW]\|"),
        [
            ...
            ("lat", lambda t, s: float(s.strip())),
            ("lat", lambda t, s: t + (float(s.strip()) * (1.0/60.0))),
            ("lat", lambda t, s: t + (float(s.strip()) * (1.0/3600.0))),
            ("lat", lambda t, s: t if s.strip() == "N" else -t),

            ("long", lambda t, s: float(s.strip())),
            ("long", lambda t, s: t + (float(s.strip()) * (1.0/60.0))),
            ("long", lambda t, s: t + (float(s.strip()) * (1.0/3600.0))),
            ("long", lambda t, s: t if s.strip() == "E" else -t)
        ]),
    (re.compile("[Cc]oord\|[0-9]+\|[0-9.-]+\|[NS]\|[0-9]+\|[0-9.-]+\|[EW]\|"),
        [
            (None, lambda t, s: None),
            ("lat", lambda t, s: float(s.strip())),
            ("lat", lambda t, s: t + (float(s.strip()) * (1.0/60.0))),
...
class University(object):
    ...
    def __fetch_date_established(self):
        ...
        for key in EST_KEYS:
            if key in d:
                try:
                    self.est = YEAR_REGEX.search(d[key]).group(1)
                except AttributeError:
                    pass
                return
    ...
    def write_to_file(self, outfilehandle):
        if (self.coords != None
                    ...
                    and self.est != None):
            data_str = "{}\t{}\n".format(self.est, self.coords.coord_str)
            outfilehandle.write(data_str)
